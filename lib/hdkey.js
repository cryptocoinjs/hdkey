 ...
 * @prop {HDUtilSign} sign
 * @prop {HDKeyToKey} toPublicKey
 * @prop {HDUtilVerify} verify
 */
...
(function (window, HDKey) {
  ...
  /** @type {HDKeyTweak} */
  Utils.privateKeyTweakAdd = async function (privateKeyCopy, IL) {
    let secp256k1 = require("secp256k1");
    return await secp256k1.privateKeyTweakAdd(privateKeyCopy, IL);
  };

  /** @type {HDKeyToKey} */
  Utils.publicKeyNormalize = async function (publicKey) {
    let secp256k1 = require("secp256k1");
    assert(secp256k1.publicKeyVerify(publicKey) === true, "Invalid public key");
  ...
  /** @type {HDKeyTweak} */
  Utils.publicKeyTweakAdd = async function (publicKeyCopy, IL) {
    let secp256k1 = require("secp256k1");
    return secp256k1.publicKeyTweakAdd(publicKeyCopy, IL, COMPRESSED);
  };
  ...
  /** @type {HDUtilSign} */
  Utils.sign = async function (hash, privateKey) {
    let secp256k1 = require("secp256k1");
    return await secp256k1.ecdsaSign(hash, privateKey).signature;
  };

  /** @type {HDKeyToKey} */
  Utils.toPublicKey = async function (privBuf) {
    let secp256k1 = require("secp256k1");
    assert(secp256k1.privateKeyVerify(privBuf) === true, "Invalid private key");
    return secp256k1.publicKeyCreate(privBuf, true);
  };

  /** @type {HDUtilVerify} */
  Utils.verify = async function (signature, hash, publicKey) {
    let secp256k1 = require("secp256k1");
    return await secp256k1.ecdsaVerify(signature, hash, publicKey);
  };
  ...
  HDKey.create = function (versions) {
    ...
    hdkey.sign = async function (hash) {
      ...
      return await Utils.sign(hash, _privateKey);
    };

    hdkey.verify = async function (hash, signature) {
      return await Utils.verify(signature, hash, hdkey.publicKey);
    };
...
/**
 * @callback HDUtilSign
 * @param {Uint8Array} hash - message to sign
 * @param {Uint8Array} privateKey
 * @returns {Promise<Uint8Array>} - signature
 */

/**
 * @callback HDUtilVerify
 * @param {Uint8Array} hash - message to verify
